...
1. **Filtrar por `aplica_si` y materializar con el script**: recorrer los
   ...
   aplica va a `no_aplican` CON su razón en lenguaje del cliente ("ya tienen
   sitio con catálogo", "no se capturó si cobran anticipo"). Nada desaparece en
   silencio.
...
`scripts/construir_propuesta.py` y el bloque 11 del validador la respalda —
no la reconstruyas a mano.
